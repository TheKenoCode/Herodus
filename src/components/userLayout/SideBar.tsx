...
const SideBar: React.FC<Props> = ({
  ...
  return (
    <div className="sticky top-0 right-0 flex-col items-center justify-between hidden w-20 h-screen lg:w-60 md:flex border-grayBorder">
      <div className="flex flex-col px-4 ">
        {menuItems.map((item, index) => {
          const isActive = pathname === item.link
          console.log(item.userAuth) // Check if the current path matches the item's link
          return (
            <>
              {item.userAuth && (
                <div
