...
export default function NavBar() {
  ...
  useEffect(() => {
    if (
      ...
      (isLoggedIn && token && isJwtExpired(token))
    ) {
      dispatch(logout());
      router.push('/login');
    }
        ...
        <div className='flex items-center justify-between w-full my-auto '>
          ...
          >
            {currentMenuItems.map((menuItem, index) => (
              <li className='my-5 animate-fade  animate-once' key={index}>
                <Link
            ...
            {status === 'authenticated' || isLoggedIn ? (
              <button
                className='w-32 py-1 text-white uppercase transition duration-500 transform border-2 rounded-full cursor-pointer border-secondary hover:scale-125 hover:bg-secondary animate-fade  animate-once'
                onClick={() => {
                  signOut();
                  dispatch(logout());
                }}
              ...
              <>
                <Link href='/auth/register'>
                  <button
                    ...
                    className='cursor-pointer border-secondary border-2 xl:mr-[-10px]  w-32 mb-2 xl:mb-0 py-1 uppercase transform text-white hover:scale-125 transition duration-500 rounded-full hover:bg-secondary animate-fade  animate-once'
                  >
                    Register
                  </button>
                </Link>
                <Link href='/auth/login'>
                  <button
                    ...
                    className='w-32 py-1 text-white uppercase transition duration-500 transform border-2 rounded-full cursor-pointer border-third hover:scale-125 hover:bg-third animate-fade  animate-once'
                  >
                    Login
                  </button>
                </Link>
